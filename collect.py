...
for nam in names:
    ...
    for tweet in sntwitter.TwitterSearchScraper(f'from:{nam}').get_items():
        ...
        #print(tweet)
        tweets_collection = tweets_candidats.append([tweet.user.username,tweet.date,tweet.rawContent,tweet.likeCount,tweet.sourceUrl,tweet.quotedTweet,tweet.outlinks,tweet.quoteCount,tweet.replyCount,tweet.retweetCount,tweet.retweetedTweet,tweet.inReplyToTweetId,tweet.inReplyToUser,tweet.mentionedUsers])
    ...
    pd.DataFrame(tweets_candidats,columns=["Username","Date","Tweet","Likes","quotecount","sourceUrl","quotedTweet","quotecount","replyCount","Retweets","retweetedTweet","inReplyToTweetId","inReplyToUser","mentionedUsers"]).to_csv('data/'+nam+'.csv')
